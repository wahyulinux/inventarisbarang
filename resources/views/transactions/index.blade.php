...
<div x-data="{ openModal: false }">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex items-center gap-3 w-full md:w-auto">
            <a href="{{ route('transactions.print', request()->query()) }}" target="_blank" class="flex-1 md:flex-none inline-flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        <form action="{{ route('transactions.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
            ...
            <div>
                ...
                <input type="month" name="bulan" class="w-full px-3 py-2 border rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ request('bulan') }}">
            </div>
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('transactions.index') }}" class="inline-flex justify-center px-4 py-2 bg-gray-100 border border-transparent rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-200 transition w-full md:w-auto">
                    Reset
                </a>
            </div>
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $t->item->name }}</div>
                            <div class="text-xs text-gray-500">{{ $t->item->sku }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            {{ $t->warehouse->name ?? '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold {{ $t->type === 'in' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ strtoupper($t->type) }}
                            </span>
                        </td>
